...
final class Settings
{
    ...
    public function get_default_settings_link()
    {
        ...
        return array(
            '<a href="' . esc_attr($this->get_default_settings_url()) . '">' . esc_html__('Settings') . '</a>',
        );
    ...
    public function plugin_action_links($links, $plugin_file, $plugin_data, $context)
    {
        ...
        if (!is_bool($this->action_links) && !is_array($this->action_links)) {
            ...
            $settings_link = array(
                '<a href="' . admin_url(esc_url($this->action_links)) . '">' . esc_html__('Settings') . '</a>',
            );
...
//		    TODO: if the option to store file as file url
    ...
    function callback_text($args)
    {
        ...
        echo '<input type="'.esc_attr($args['type']).'" class="'.esc_attr($args['size']).'-text '.esc_attr($args['class']).'" id="'.esc_attr($args['section']).'['.esc_attr($args['id']).']"  name="'.esc_attr($args['name']).'" value="'.esc_attr($args['value']).'"   '.$this->get_markup_placeholder($args['placeholder']).'  />'.$this->get_field_description($args);
    ...
    function callback_number($args)
    {
        ...
        echo '<input type="'.esc_attr($args['type']).'" class="'.esc_attr($args['size']).'-text" id="'.esc_attr($args['section']).'['.esc_attr($args['id']).']" name="'.esc_attr($args['name']).'" value="'.esc_attr($args['value']).'" '.$this->get_markup_placeholder($args['placeholder']).' '.$min.' '.$max.' '.$step.'/>'.$this->get_field_description($args);
    ...
    {
        echo '<fieldset>';
        echo '<label for="'.esc_attr($args['section']).'['.esc_attr($args['id']).']">';
        echo '<input type="checkbox" class="checkbox" id="'.esc_attr($args['section']).'['.esc_attr($args['id']).']" name="'.esc_attr($args['name']).'" value="1" '.checked(esc_attr($args['value']), '1', false).' />';
        echo esc_html($args['desc']).'</label>';
        echo '</fieldset>';
    }
    ...
    function callback_multicheck($args)
    {
        ...
        foreach ($args['options'] as $key => $label) {
            ...
            echo '<label for="'.esc_attr($args['section']).'['.esc_attr($args['id']).']['.esc_attr($key).']">';
            echo '<input type="checkbox" class="checkbox" id="'.esc_attr($args['section']).'['.esc_attr($args['id']).']['.esc_attr($key).']" name="'.esc_attr($args['name']).'['.esc_attr($key).']" value="'.esc_attr($key).'" '.checked(esc_attr($checked), $key, false).' />';
    ...
    function callback_radio($args)
    {
        ...
        foreach ($args['options'] as $key => $label) { ?>
            ...
            <label for="<?php echo esc_attr($args['section']) ?>[<?php echo esc_attr($args['id']) ?>][<?php echo esc_attr($key) ?>]">
                <input type="radio" class="radio" id="<?php echo esc_attr($args['section']) ?>[<?php echo esc_attr($args['id']) ?>][<?php echo esc_attr($key) ?>]" name="<?php echo esc_attr($args['name']) ?>" value="<?php echo esc_attr($key) ?>" <?php checked(esc_attr($args['value']), $key, true) ?> />
    ...
    function callback_select($args)
    {
        ...
        echo sprintf('<select class="%1$s-text %5$s" name="%4$s" id="%2$s[%3$s]">', esc_attr($args['size']), esc_attr($args['section']), esc_attr($args['id']), esc_attr($args['name']), esc_attr($args['class']));

        foreach ($args['options'] as $key => $label) {
            echo
                sprintf('<option value="%1s"%2s>%3s</option>',
                    esc_attr($key),
                    selected(esc_attr($args['value']), $key, false),
                    esc_html($label)
                );
        }
    ...
    function callback_textarea($args)
    {
        ...
        <textarea
                ...
                <?php echo $this->get_markup_placeholder($args['placeholder']) ?>
    ...
    function callback_file($args)
    {
        ...
        echo sprintf('<input type="url" class="%1$s-text wpsa-url" id="%2$s[%3$s]" name="%5$s" value="%4$s"/>', esc_attr($args['size']), esc_attr($args['section']), esc_attr($args['id']), esc_attr($args['value']), esc_attr($args['name']));
        echo '<input type="button" class="button boospot-browse-button" value="' . esc_attr($label) . '" />';
        echo $this->get_field_description($args);
    ...
    public function callback_media($args)
    {
        ...
        // Print HTML field
        echo '
                <div class="upload" style="max-width:' . esc_attr($max_width) . ';">
                    <img data-src="' . esc_attr($default_image) . '" src="' . esc_attr($src) . '" ' . $image_style . '/>
                    <div>
                        <input type="hidden" name="' . esc_attr($args['name']) . '" id="' . esc_attr($args['name']) . '" value="' . esc_attr($value) . '" />
                        <button type="submit" class="boospot-image-upload button">' . esc_html($text) . '</button>
    ...
    function callback_password($args)
    {
        ...
        echo sprintf('<input type="password" class="%1$s-text" id="%2$s[%3$s]" name="%5$s" value="%4$s"/>', esc_attr($args['size']), esc_attr($args['section']), esc_attr($args['id']), esc_attr($args['value']), esc_attr($args['name']));
        echo $this->get_field_description($args);
    ...
    {
        echo sprintf('<input type="text" class="%1$s-text wp-color-picker-field" data-alpha="true" id="%2$s[%3$s]" name="%6$s" value="%4$s" data-default-color="%5$s" />', esc_attr($args['size']), esc_attr($args['section']), esc_attr($args['id']), esc_attr($args['value']), esc_attr($args['default']), esc_attr($args['name']));
        echo $this->get_field_description($args);
    ...
    function show_navigation()
    {
        ...
        foreach ($this->settings_sections as $tab) {
            ...
            echo sprintf('<a href="%3$s&tab=%1$s" class="nav-tab %4$s" id="%1$s-tab">%2$s</a>', esc_attr($tab['id']), esc_html($tab['title']), $settings_page, esc_attr($active_class));
        }
